...
export function getMonthDays(year: string, month: string): number {
  ...
  return ([0, 31, isLeapYear(Number(year)) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] as number[])[month as any]
}
 ...
 * @return {string}
 */
export function date2Str(date: Date, split = '-'): string {
 ...
 */
export function getDay(i = 0): string {
  let date = new Date()
...
export function getYearWeek(year: string, month: string, date: string): number {
  ...
  const dataNumber = Math.round((dateNow.valueOf() - dateFirst.valueOf()) / 86_400_000)
